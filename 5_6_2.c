...
int main(int N) {
  ...
  for(i = 0; i<N; i++)
  {
    pthread_create(pid1++, NULL, Producer, (void*) &stat);stat++;
    pthread_create(cid1++, NULL, Consumer, (void*) &stat);stat++;
...
void *Consumer(void *arg) {
  ...
  while(i < NUM_ITER) {
    ...
    /* the following two lines just prints a bar showing current buffer fill */
    j=0; printf("(Consumer id: %d, semaphore full is %d)  \t", *navn, semvalfu);
